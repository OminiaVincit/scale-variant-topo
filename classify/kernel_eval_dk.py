...
def read_train_test(filename, selected_lbs, rtidx):
    """
    ...
    with open(filename, 'r') as rf:
        ...
        for line in lines:
            ...
            if 'original' in line:
                ...
                lb = 4
            if lb not in selected_lbs:
                continue
            lbs.append(lb)
            n = len(lbs) - 1
            idx = int(re.search('index_([0-9]+)_', line).groups()[0]) - 1
            if idx in rtidx:
                testls.append(n)
            else:
                trainls.append(n)
    ...
    train_index = np.array(trainls).astype(np.int32)
    test_index = np.array(testls).astype(np.int32)
    y = np.array(lbs).astype(np.int32)
    print('y: ', y.shape, 'train: ', train_index.shape, 'test: ', test_index.shape)
    
    return y, train_index, test_index
...
if __name__ == '__main__':
    ...
    parser.add_argument('--log', type=str, default='logdkconf')
    parser.add_argument('--exppath', type=str, default=EXPPATH)
    parser.add_argument('--setting', type=str, default='barlist_config_join_10_d_1.txt')
    ...
    for n in range(args.nums):
        ...
        rtidx = np.random.choice(10, 5, replace=False)
        y, train_index, test_index = read_train_test(setting_file, selected_lbs, rtidx)
        print('trainls = {}, testls ={}, rtidx '.format(len(train_index), len(test_index)), rtidx)
        train_sc, test_sc = svc_classify(kerX, y, train_index, test_index)
        global_train.append(train_sc)
        global_test.append(test_sc)
        logger.debug('n={}, local score: train={}, test={}'.format(n, train_sc, test_sc))
        ...
        logger.debug('n={}, dk, glocal score (mean, std): train=( {}, {} ), test=( {}, {} )'.format(n, \
